...
Parameter $n$, wenn sie die Dichtefunktion 
$$
f(x)=\left\{
\begin{array}{ll}
C_n\cdot x^{\frac{n}{2}-1}\exp(-\frac{x}{2}) & \text{ wenn }x \ge 0\\
0 & \text{ sonst.}
\end{array}
\right.
$$
hat. Der Parameter $n$ wird Zahl der Freiheitsgrade (engl. degrees of freedom) 
genannt. Die Konstante $C_n$ dient nur der Normierung, ihre genaue 
...
gängige Kurzschreibweise für eine $\chi^2$-verteilte Zufallsvariable mit $n$ 
Freiheitsgraden ist $X\sim \chi^2_n$.
...
Normalverteilung modellieren? Folgt die Zahl der Kunden, die
...
::: { .callout-tip collapse="true"}
## Beispiel: Anzahl der Kundenanrufe

Es soll getestet werden, ob die Anzahl 
von Kunden, die innerhalb von 15 Minuten bei
...
$$
Da Betrüger diese Gesetzmäßigkeit nicht gut in gefälschte
...
Die Größe von Städten (meist gemessen durch die Einwohnerzahl im
...
jeden beliebigen Wert setzen, der größer ist als die Einwohnerzahl von
Tokyo.
```{r}
