...
In der Programmierumgebung R können Mengen durch Vektoren repräsentiert werden,
zumindest wenn die Anzahl der Elemente in den Mengen nicht zu groß ist.
Die Vektoren können entweder Zahlen oder Zeichen(ketten) als
Elemente enthalten. 
...
C <- 2
```
Das Ereignis "A und B" entspricht der Schnittmenge
$A\cap B$. In R finden wir dieses Ereignis mit der
Funktion `intersect`.
```{r}
intersect(A,B)
```
Das Ereignis "A oder B" entspricht der Vereinigungsmenge
$A\cup B$. Dazu brauchen wir die R-Funktion `union`.
```{r}
union(A,B)
```
Das Komplementärereignis "nicht A" enthält alle Elemente
aus $\Omega$, die nicht in der Menge $A$ enthalten sind,
also die Mengendifferenz.
```{r}
setdiff(Omega,A)
```
