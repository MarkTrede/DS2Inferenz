...
D <- data.frame(x=rep(x,3), 
                y=c(dnorm(x,-1,1),dnorm(x,1,1),dnorm(x,5,1)),
                m=rep(factor(c(-1,1,5)),each=500))
ggplot(D,aes(x,y,colour=m))+
    geom_line()+
    scale_colour_discrete(expression(mu))+
    ylab("Dichte")+
...
D <- data.frame(x=rep(x,3), 
                y=c(dnorm(x,0,sqrt(0.3)),
                    dnorm(x,0,1),
                    dnorm(x,0,sqrt(3))),
                m=rep(factor(c(0.3,1,3)),each=500))
ggplot(D,aes(x,y,colour=m))+
    geom_line()+
    scale_colour_discrete(expression(sigma^2))+
    ylab("Dichte")+
    theme(legend.position = "bottom")
```
...
Die Paretoverteilung hat zwei Parameter, und zwar $c>0$ und $\alpha>0$.
Die Dichte lautet
$$
f(x)=\left\{
\begin{array}{ll}
0 & \text{ für }x<c\\
\frac{\alpha}{c}\left(\frac{c}{x}\right)^{\alpha+1}& \text{ für }x\ge c.
\end{array}
\right.
$$
Für $c=1$ und $\alpha=2$ ergibt sich folgendes Bild:
```{r echo=FALSE}
x <- seq(1,5,length=200)
alpha <- 2
cc <- 1
D <- data.frame(x=x,
                y=(alpha/cc)*(cc/x)^(alpha+1))
ggplot(D, aes(x=x,y=y))+
    geom_line()+
    xlim(0,5)+
    ylab("Dichte")+
    ggtitle("Dichte der Paretoverteilung")
...
dass die Paretoverteilung als Untergrenze den Parameter $c>0$ hat,
...
\begin{array}{ll}
0 & \text{ für }x<c\\
1-\left(\frac{c}{x}\right)^\alpha & \text{ für }x\ge c.
\end{array}
\right.
$$
...
$$
\begin{align*}
E(X) &= \frac{\alpha c}{\alpha-1}\\
Var(X) &= \frac{\alpha c^2}{(\alpha-2)(\alpha-1)^2}.
\end{align*}
$$
Der Erwartungswert ist nur definiert, wenn der $\alpha>1$ ist,
die Varianz ist sogar nur dann definiert, wenn $\alpha>2$ ist.