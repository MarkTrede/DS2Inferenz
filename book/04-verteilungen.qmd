...
- [Liste univariater Wahrscheinlichkeitsverteilungen](https://de.wikipedia.org/wiki/Liste_univariater_Wahrscheinlichkeitsverteilungen).

Die entsprechende [englischsprachige Seite](https://en.wikipedia.org/wiki/List_of_probability_distributions) listet noch mehr Verteilungen auf, stellt ihre
...
wird ausführlich in dem Bachelor-Modul **Advanced Statistics** behandelt.
...
Sei $X\sim Po(\lambda)$ 
die Zahl der Flugpassagiere, die innerhalb einer Viertelstunde am Terminal
...
Passagiere ankommen, beträgt
```{r}
dpois(6, lambda=8)
```
Die Wahrscheinlichkeit, dass zwischen 14:30 und 14:35 mehr als 7 
Passagiere eintreffen, ist $P(Y>10)=1-P(Y\le 10)$,
```{r}
1 - ppois(7, lambda=4)
```
:::
...
in @ch-intervallschaetzung und @ch-hypothesentests behandelt werden.
...
anderen Bestandteile der Dichte dienen nur der Normierung. Ein Plot der Dichte
...
lines(x,dnorm(x,1,1),col="green")
lines(x,dnorm(x,5,1),col="blue")
legend("topright",
       fill=c("red","green","blue"),
       legend=c(expression(mu==1),expression(mu==2),expression(mu==3)))
```
...
lines(x,dnorm(x,0,sqrt(3)),col="blue")
legend("topright",
       fill=c("red","green","blue"),
...
beliebigen Normalverteilung selbst dann auswerten, wenn man nur die
Verteilungsfunktion des Standardnormalverteilung kennt. In alten Lehrbüchern
zur Statistik finden Sie deshalb im Anhang Tabellen mit den Funktionswerten
...
<img src="images/AdobeStock_458950051b_AttentionSign.jpeg" align="right" width=10%" style="padding-right:15px;padding-top:5px;padding-bottom:15px;"/>
...
Die Paretoverteilung kann in vielen Anwendungen extreme Ereignisse gut
beschreiben. Beispielsweise folgt die Vermögensverteilung bei den Superreichen
einer Paretoverteilung. Auch extreme Aktienrenditen können gut durch eine
...
\begin{array}{ll}
0 & \text{ für }x<x_{min}\\
\frac{k}{x_{min}}\left(\frac{x_{min}}{x}\right)^{k+1}& \text{ für }x\ge x_{min}.
\end{array}
\right.
$$
...
curve((k/xmin)*(xmin/x)^(k+1),
      ...
      ylim=c(0,2.2),
      xlim=c(0,5))
lines(c(xmin,xmin),c(0,2), lty="dotted")
```
...
Duch Integration der Dichte erhält man die Verteilungsfunktion.
$$
F(x)=\left\{
\begin{array}{ll}
0 & \text{ für }x<x_{min}\\
1-\left(\frac{x_{min}}{x}\right)^k & \text{ für }x\ge x_{min}.
\end{array}
\right.
$$
...
die Varianz ist sogar nur dann definiert, wenn $k>2$ ist.

In R gibt es die Funktionen für die Dichte, Verteilungsfunktion und
Quantilfunktion nur in Zusatzpaketen. Installieren und aktivieren
Sie das Paket `distributionsrd`. Nun stehen die Funktionen
...
Wie groß ist die Wahrscheinlichkeit, dass der zufällig
ausgewählte Haushalt ein Einkommen von mehr als 1 Million
Euro hat?
...
Mit einer Rechteckverteilung kann beispielsweise die
...
\begin{array}{ll}
0 & \text{ für }x<a\\
\frac{x-a}{b-a}& \text{ für }a\le x\le b\\
1 & \text{ für }x>b.\\
\end{array}
\right.
$$
...
Standardnormalverteilung. Bei sinkenden Freiheitsgraden werden die Flanken der Verteilung immer stärker, d.h. es gibt immer mehr extreme Werte (Ausreißer). 
...
ernsthaft empirisch untermauert.) Die Tagesrendite folgt also $X\sim t_3$.
...
Größen in der Popoulation verteilt? Wie sieht die Einkommensverteilung
...
Wie hoch ist der Anteil der Kunden, die ihren Versicherungsvertrag
