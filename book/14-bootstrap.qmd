...
keinen vorgefertigten Test gibt. Ökonomen sollten also nicht nur
vorgefertigte Tests verstehen und anwenden können, sondern auch in 
der Lage sein, neue Tests zu entwickeln.

In diesem Kapitel wird ein computer-gestützter Ansatz für
...
Bootstrap-Populationsverteilung. Wir nennen sie $X^b$ 
(für Bootstrap) mit den Ausprägungen
$$
x_i^b=x_i+\mu_0-\bar x,\quad i=1,\ldots,n.
$$
Die Wahrscheinlichkeitsfunktion von $X^b$ versieht jeden dieser Werte
mit der Wahrscheinlichkeit $1/n$. Der Erwartungswert von $X^b$
ist (vgl. @sec-empverteilung)
$$
\begin{align*}
E(X^b)&=\frac{1}{n}\sum_{i=1}^n x_i^b\\
&= \frac{1}{n}\sum_{i=1}^n (x_i+\mu_0-\bar x)\\
...
$X^b$ (denn die ist bekannt). Das ist der Kern der Bootstrap-Methode,
...
angepasste empirische Verteilung der Stichprobe $X^b$ ersetzt 
...
$X^b$ gezogen werden. Diese Ziehungen nennt 
man **Pseudo-Stichproben** oder **Resamples**. 
Eine Pseudo-Stichprobe $X^b_1,\ldots,X^b_n$
unter der Nullhypothese lässt sich mit der 
R-Funktion `sample` ziehen. 
```{r}
xb <- sample(x + mu0 - mean(x), size=n, replace=TRUE)
```
Man zieht also aus der an die Nullhypothese angepassten
Original-Stichprobe wie aus einer Urne
$n$ Elemente. Nach jedem Zug wird das gezogene Element wieder zurückgelegt
...
In jedem Schleifendurchlauf wird eine Pseudo-Stichprobe vom
Umfang $n$ aus der Bootstrap-Population $X^b$ gezogen. 
Aus jeder Pseudo-Stichprobe berechnet man den Wert der 
...
wird eine Pseudo-Stichprobe `xb` gezogen. Aus ihr wird die Teststatistik
...
durch das Ziehen der Pseudo-Stichproben. Er wird jedoch
immer kleiner, wenn man die Zahl der Durchläufe $B$ erhöht.
...
der $X$-Stichprobe um $\bar y-\bar x$ verschoben. Beide
Bootstrap-Populationen haben dann den Erwartungswert $\bar y$.
...
for(b in 1:B){
  ...
  xb <- sample(x + mean(y) - mean(x), size=m, replace=TRUE)
...
$$
x_i^b=x_i\cdot\sqrt{\frac{0.5}{s^2}}.
$$
...
for(b in 1:B){
  ...
  xb <- sample(x * sqrt(0.5/teststat), size=n, replace=TRUE)
  teststatb[b] <- var(xb)
...
der Kunden, die innerhalb einer Viertelstunde bei einer
