...
::: callout-note
Die **Kovarianz** zweier gemeinsam verteilter Zufallsvariablen 
$X$ und $Y$ ist
$$
...
als im Erwartungswert. Das Produkt ist immer dann positiv, wenn
...
Portfolio-Management, Risiko-Management, Banken und Versicherung. 

Bei einer linearen Transformation von $X$ und $Y$ gilt
$$
Cov(aX+b,cY+d)=a\,c\, Cov(X,Y),
$$
d.h. die additiven Konstanten $b$ und $d$ fallen einfach weg, während
die multiplikativen Kontanten vor die Kovarianz gezogen werden können.
...
verallgemeinern dies anschließend Schritt für Schritt.
...
ist eine Anwendung der ersten binomischen Formel ($(a+b)^2=a^2+2ab+b^2$).
...
im allgemeinen *nicht* nur die Summe ihrer Varianzen! Es gibt einen
...
Ein Investor möchte sein Vermögen (nur) in diesen beiden 
Assets anlegen. Er fragt seine Beraterin, wie hoch der
Anteil des Vermögens sein soll, der in Wertpapier A angelegt
wird, wenn das Risiko minimiert werden soll. Das Risiko wird
gewöhnlich durch die Varianz (bzw. die Standardabweichung)
gemessen. Die Beraterin
kennt die folgenden Kennzahlen (woher sie diese Informationen
hat, sei an dieser Stelle dahingestellt):
$$
\begin{align*}
E(X) &= 0.02\\
E(Y) &= 0.06\\
...
\end{align*}
$$
Die erwartete Rendite von Wertpapier A ist also niedriger,
sie beträgt nur 2 Prozent per annum (p.a.). Dafür ist Papier A
jedoch auch weniger riskant, die Standardabweichung beträgt
0.1, wohin gegen Wertpapier 2 eine Standardabweichung von
...
Anteil. Die Gesamtrendite dieses kleinen Portfolios ist
$$
R=aX+(1-a)Y.
$$
...
$$
Das Risiko der Portfoliorendite, gemessen durch seine Varianz, beträgt
$$
\begin{align*}
...
höher ist als der kleinere der beiden Einzelrenditen, und gleichzeitig 
eine Varianz, die niedriger ist als die kleinere der beiden 
Einzelvarianzen! Es wäre also geradezu dumm, sein ganzes Vermögen
...
einzelnen Erwartungswerten und Varianzen (und Kovarianzen!) verhält. 
...
ist jedoch in der Notation komplexer. In dem (englischsprachigen) 
Bachelor-Modul **Advanced Statistics** werden multivariate
...
\end{align*}
$$
Das die letzte Umformung korrekt ist, erkennt man am leichtesten,
wenn man alle Kovarianzen in Matrixform anordnet:
$$
\begin{array}{ccccc}
...
Nebendiagonale hinzu etc. Da alle Elemente oberhalb
...
bleibt also im Fall der Unabhängigkeit
$$
...
\end{align*}
$$
Daher gilt für Erwartungswert und Varianz der
binomialverteilten Zufallsvariable $X\sim B(n,\pi)$,
$$
\begin{align*}
E(X) &= n\pi \\
Var(X) &= n\pi(1-\pi).
...
benötigt werden, können Sie in den Wahlmodulen
...
alle gleich (nämlich 0.2) sind, vereinfacht sich die Varianzformel
zu
$$
...
(nämlich 4.6 Prozent im Vergleich zu 2 Prozent).
:::
