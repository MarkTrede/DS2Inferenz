...
In der folgenden Abbildung, die Sie mit der Maus bewegen können
...
library(mvtnorm)
library(rgl)
xx <- yy <- seq(-5,5,length=101)
zz1 <- outer(xx,
             yy, 
             function(z1,z2) dmvnorm(cbind(z1,z2),
                                     mean=c(2,2),
                                     sigma=matrix(1.5*c(1,0.4,0.4,1),2,2)))
zz2 <- outer(xx,
             yy, 
             function(z1,z2) dmvnorm(cbind(z1,z2),
                                     mean=c(-2,1),
                                     sigma=matrix(1.5*c(1,-0.5,-0.5,1),2,2)))
zz <- 0.6*zz1 + 0.4*zz2
persp3d(xx,yy,zz,col="light green")
rglwidget()
close3d()
```
