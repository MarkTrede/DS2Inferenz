...
pVERTEILUNG(q, PARAMETER) # F(q), Vert.fkt
qVERTEILUNG(p, PARAMETER) # Q(x), Quantilfkt.

rVERTEILUNG(n, PARAMETER) # Zufallszahlen
```
...
Aus der Population der Erwerbspersonen wird zufällig
eine Person ausgewählt und befragt, ob sie arbeitslos ist; 
sei $X$ der Status
$$
X=\left\{
\begin{array}{ll}
...
\item $X$ ist die Anzahl der Erfolge bei $n$ Ziehungen
...
\begin{center}
$\color{red}\bullet\color{black}~~\circ~~\circ~~\color{red} \bullet\color{black}~~\circ$\\
$\circ~~\color{red} \bullet\color{black}~~\circ~~\circ~~\circ$
\end{center}
...
\item Eine Zufallsvariable $U$ heißt standardnormalverteilt, wenn sie für $u\in \mathbb{R}$ die folgende Dichte hat
$$
\varphi(u) =\frac{1}{\sqrt{2\pi}}\color{red}\exp\color{black}
\left(\color{red}-\color{black}\frac{1}{2}\color{red}u^2\color{black}\right)
$$
\item Notation: $U\sim N(0,1)$
\end{itemize}
...
$\mu\in\mathbb{R}$ und $\sigma^2>0$, falls für $x\in\mathbb{R}$
$$
f_X(x) =\frac{1}{\sqrt{2\pi}\sigma}\color{red}\exp\color{black}
\left(\color{red}-\color{black}\frac{1}{2}\color{red}
\left(\color{black}\frac{x-\mu}{\sigma}\color{red}\right)^2\color{black}\right)
$$
\item Notation: $X\sim N(\mu,\sigma^2)$
\end{itemize}
...
plot(x,dnorm(x, mean=-1, sd=1),
     ...
     ylim=c(0, 0.45),
     ylab="Dichten",
     main="Einfluss von mu (wenn sigma=1)")
...
plot(x, dnorm(x, mean=0, sd=sqrt(0.3)),
     ...
     ylim=c(0, 0.8),
     ylab="Dichten",
     main="Einfluss von sigma^2 (wenn mu=0)")
...
$$
\item $p$-Quantil: $x_p=\mu +\sigma \cdot u_p$
\item Erwartungswert und Varianz
\begin{align*}
E(X)&=\mu\\
Var(X)&=\sigma^2
...
f_X(x)=\left\{ 
\begin{array}{ll}
\frac{1}{b-a} & \quad a\le x\le b\\[2ex]
0 & \quad \text{sonst}
\end{array} \right.
$$
