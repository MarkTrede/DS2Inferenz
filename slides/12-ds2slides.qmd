...
text(-2.8,0.05,"2.5%",cex=1.8)
text(2.8,0.05,"2.5%",cex=1.8)
text(-0.05,0.18,"95%",cex=1.8)
```
...
jetzt nicht mehr leicht zu beantworten
\end{itemize}
...
\item Festlegung des kritischen Bereichs, wenn $H_0:\mu\le\mu_0$
\end{itemize}
...
\item Wenn $T>u_{1-\alpha}$ ist, lehnt man $H_0:\mu\le\mu_0$ ab
...
\item Wenn $T< -u_{1-\alpha}$ ist, lehnt man $H_0:\mu\ge\mu_0$ ab
...
# Erwartungswerttests
\framesubtitle{Einseitige Tests}
Wahl der Nullhypothese:\medskip
\begin{itemize}
...
# Erwartungswerttests
\framesubtitle{Einseitige Tests}
\begin{exampleblock}{Beispiel: Einseitiger Test}
Qualitätskontrolle:
\begin{itemize}
\item Gerstelieferungen an eine Brauerei
\item Sei $X\sim N(\mu,\sigma^2)$ der Kohlenhydratanteil der Gerste
\item Ziehung einer einfachen Stichprobe $X_1,\ldots,X_{10}$ aus der Lieferung
\item Die Brauerei möchte sicher sein, dass $\mu>61$ Prozent ist
\item Ihre Hypothesen sind daher
$$
H_0:\mu\le 61,\qquad H_1:\mu>61
$$
\item Die Lieferung wird nicht angenommen, wenn $H_0$ nicht verworfen wird
...
(Könnte der Wert auch zufällig zustande gekommen sein?)
...
die Nullhypothese verworfen. Die Alternative (d.h. $\mu>61$)
wird statistisch untermauert.
...
\item Hypothesen
\begin{align*}
H_0:\mu_X=\mu_Y   && H_0:\mu_X\ge \mu_Y && H_0:\mu_X\le \mu_Y\\
...
\framesubtitle{Vergleich zweier Erwartungswerte}
\begin{itemize}
\item Stichproben $X_1,\ldots,X_m$ und $Y_1,\ldots,Y_n$
\item Approximativ gilt $\bar X\sim N(\mu_X,\sigma_X^2)$
und $\bar Y\sim N(\mu_Y,\sigma_Y^2)$
\item Teststatistik
$$
...
\item Spendenwahrscheinlichkeiten $\pi_X$, $\pi_Y$
\item Hypothesen
\begin{align*}
H_0:\pi_X&\ge \pi_Y\\
H_1:\pi_X&<\pi_Y
...
\end{center}
\bigskip
\item Punktschätzer der Spendenwahrscheinlichkeiten
\begin{align*}
...
&= -2.615.
\end{align*}
\item p-Wert: $\Phi(-2.615)=0.004461$ (=\texttt{pnorm(-2.615)})
\end{itemize}
\end{exampleblock}
