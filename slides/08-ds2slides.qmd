...
beobachteten Daten
\item Die tatsächlich beobachteten Daten $x_{1},\ldots ,x_{n}$ nennt
man Realisierung der Stichprobe oder konkrete Stichprobe
\end{itemize}
...
\item Man zieht "mit Zurücklegen", damit die $X_{1},\ldots ,X_{n}$ unabhängig sind
\item Sei $X_{i}$ das Einkommen der $i$-ten befragten Person
\item Die Stichprobe ist $X_1,\ldots,X_{100}$
\item Die tatsächlich erhobenen Daten sind $x_{1},\ldots ,x_{100}$\\
(konkrete Stichprobe)
\end{itemize}
\end{exampleblock}
...
\item Im Allgemeinen kennt man die Verteilung der Population nicht
...
\framesubtitle{Verteilung}
```{r eval=FALSE}
library(distributionsrd) # für Paretoverteilung
n <- 50
xmin <- 1
k <- 5
